...
--set prometheusOperator.admissionWebhooks.patch.image.registry="$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/k8s" \
--set prometheusOperator.prometheusConfigReloader.image.registry="$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/quay" \
--set prometheusOperator.image.registry="$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/quay" \
--set kube-state-metrics.image.registry="$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/k8s" \
--set alertmanager.alertmanagerSpec.image.registry="$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/quay" \
--set prometheus.prometheusSpec.image.registry="$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/quay" \
--set grafana.enabled=$ENABLED
