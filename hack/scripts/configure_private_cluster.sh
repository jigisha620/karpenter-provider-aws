...
aws ec2 create-vpc-peering-connection --vpc-id "vpc-0ee350bd834ac1542" --peer-vpc-id "${VPC_ID}"
...
SUBNET_CONFIG=$(aws ec2 describe-subnets --filters Name=vpc-id,Values="vpc-0ee350bd834ac1542" Name=tag:aws-cdk:subnet-type,Values=Private --query "Subnets")
echo $SUBNET_CONFIG | jq '.[].SubnetId' -r |
while read -r subnet;
do
  ROUTE_TABLE_ID=$((aws ec2 describe-route-tables --filters Name=vpc-id,Values="vpc-0ee350bd834ac1542" Name=association.subnet-id,Values=$subnet --query "RouteTables[0].RouteTableId") | jq -r)
...
do
  echo "do something with $routeTableId";
