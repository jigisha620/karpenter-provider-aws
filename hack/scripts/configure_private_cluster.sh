...
aws ec2 create-vpc-peering-connection --vpc-id "${CODEBUILD_VPC}" --peer-vpc-id "${VPC_ID}"
...
echo CODEBUILD_SUBNET_CONFIG="$subnet_config" >> "$GITHUB_ENV"
...
do
  ...
  aws ec2 create-route --route-table-id "$ROUTE_TABLE_ID" --destination-cidr-block "${CIDR_BLOCK}" --vpc-peering-connection-id "$vpc_peering_connection_id"
done
