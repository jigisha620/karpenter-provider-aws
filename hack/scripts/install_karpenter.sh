...
if (( "$PRIVATE_CLUSTER" == 'true' )); then
  CHART=$(find . -name karpenter*)
  ADDITIONAL_FLAGS="--set controller.image.repository=$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/karpenter/snapshot/controller --set controller.image.tag=643689a11697d47a19bcd98d25840589c4b31548 --set controller.image.tag=\"\""
fi

# Remove service account annotation when dropping support for 1.23
helm upgrade --install karpenter "${CHART}" \
  -n kube-system \
  --version "0-643689a11697d47a19bcd98d25840589c4b31548" \
  --set logLevel=debug \
